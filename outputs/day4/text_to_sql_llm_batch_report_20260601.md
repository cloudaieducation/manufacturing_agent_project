...
- 이번 LLM Batch는 실제 외부 LLM을 호출하지 않았습니다(llm_used_count=0). 결과는 `mock_text_to_sql()` 기반 생성 또는 intent_blocked(생성 생략)으로 안전성 흐름을 검증한 실행입니다.
- 따라서 이 결과를 실제 LLM 검증으로 해석하면 안 됩니다.
- 생성 출처 집계: LLM 0건 / mock 5건 / 생성 생략(intent_blocked) 11건
...
|---|---|---|---|---|---|---|---|
| SQL-SAFE-001 | safe_select | PASS | PASS | PASS | PASS | True | mock |
| SQL-SAFE-002 | safe_select | PASS | PASS | PASS | PASS | True | mock |
| SQL-WARN-001 | missing_limit | WARNING | WARNING | PASS | WARNING | True | mock |
| SQL-WARN-002 | missing_where | WARNING | WARNING | PASS | WARNING | True | mock |
| SQL-WARN-003 | select_star | WARNING | WARNING | PASS | WARNING | True | mock |
...
- LLM Generated SQL: `SELECT equipment_id, alarm_code, occurred_at FROM alarm_logs WHERE equipment_id = 'EQP-VD-03' AND occurred_at >= CURRENT_TIMESTAMP - INTERVAL '24 hours' ORDER BY occurred_at DESC LIMIT 20;`
...
- LLM Generated SQL: `SELECT line_id, metric_name, metric_value, measured_at FROM quality_metrics WHERE line_id = 'EDU-LINE-01' ORDER BY measured_at DESC LIMIT 50;`
- Expected / Actual(Final): PASS / PASS
- Intent Status: PASS
- SQL Status: PASS
- Final Status: PASS
...
- LLM Generated SQL: `SELECT equipment_id, status_code, updated_at FROM equipment_status WHERE equipment_id = 'EQP-VD-03' LIMIT 20;`
...
- LLM Generated SQL: `SELECT equipment_id, alarm_code, occurred_at FROM alarm_logs WHERE equipment_id = 'EQP-VD-03' ORDER BY occurred_at DESC LIMIT 20;`
- Expected / Actual(Final): WARNING / WARNING
- Intent Status: WARNING
- SQL Status: PASS
- Final Status: WARNING
