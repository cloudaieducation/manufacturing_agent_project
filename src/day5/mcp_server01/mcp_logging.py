# -*- coding: utf-8 -*-
"""
Day5 mcp_server01 - MCP Logging (안전한 JSONL 호출 이력 저장)
...
[전체 흐름에서의 위치]
    ...
    [mcp_logging.log_event]  ← sanitize_log_payload 로 안전 필드만 추려서
        ↓
    outputs/day5/mcp_server01/logs/mcp_server01.jsonl  (utf-8, 한 줄=한 이벤트)
...
from day5.mcp_server01.security import mask_sensitive_text, sanitize_arguments
...
# ---------------------------------------------------------------------------
# [프로젝트 루트 계산]
#   이 파일 위치: <root>/src/day5/mcp_server01/mcp_logging.py
#   parents[0]=mcp_server01, [1]=day5, [2]=src, [3]=<root>
...
# 기본 로그 파일명.
LOG_FILE_NAME = "mcp_server01.jsonl"
...
def get_log_dir():
    ...
    [반환]
        ...
        - 없으면 프로젝트 루트 기준 outputs/day5/mcp_server01/logs.
    ...
    return _PROJECT_ROOT / "outputs" / "day5" / "mcp_server01" / "logs"


def get_log_path():
    """로그 파일 전체 경로(Path)를 돌려준다(get_log_dir()/mcp_server01.jsonl)."""
...
def log_event(event_type, payload):
    ...
    [동작]
        ...
        3) outputs/.../mcp_server01.jsonl 에 BOM 없는 utf-8 한 줄로 append.
