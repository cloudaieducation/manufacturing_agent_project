# -*- coding: utf-8 -*-
"""
Day5 mcp_server_final - MCP Logging (안전한 JSONL 호출 이력 저장)
...
[전체 흐름에서의 위치]
    ...
    [mcp_logging.log_event]  ← sanitize_log_payload 로 안전 필드만 추려서
        ↓
    outputs/day5/mcp_server_final/logs/mcp_server_final.jsonl  (utf-8, 한 줄=한 이벤트)
...
from day5.mcp_server_final.security import mask_sensitive_text, sanitize_arguments
...
# ---------------------------------------------------------------------------
# [프로젝트 루트 계산]
#   이 파일 위치: <root>/src/day5/mcp_server_final/mcp_logging.py
#   parents[0]=mcp_server_final, [1]=day5, [2]=src, [3]=<root>
#   → parents[3] 이 프로젝트 루트다(현재 작업 디렉터리와 무관).
_PROJECT_ROOT = Path(__file__).resolve().parents[3]

# 로그 디렉터리를 바꿀 때 쓰는 환경변수 이름.
# [주의] 이 변수의 '값'은 로그/콘솔에 절대 출력하지 않는다(이름만 코드/문서에 노출).
LOG_DIR_ENV = "MCP_SERVER_FINAL_LOG_DIR"

# 기본 로그 파일명.
LOG_FILE_NAME = "mcp_server_final.jsonl"
...
def get_log_dir():
    ...
    [반환]
        - 환경변수 MCP_SERVER_FINAL_LOG_DIR 가 설정돼 있으면 그 경로.
        - 없으면 프로젝트 루트 기준 outputs/day5/mcp_server_final/logs.
    ...
    return _PROJECT_ROOT / "outputs" / "day5" / "mcp_server_final" / "logs"


def get_log_path():
    """로그 파일 전체 경로(Path)를 돌려준다(get_log_dir()/mcp_server_final.jsonl)."""
...
def log_event(event_type, payload):
    ...
    [동작]
        ...
        3) outputs/.../mcp_server_final.jsonl 에 BOM 없는 utf-8 한 줄로 append.
