# -*- coding: utf-8 -*-
"""
Day5 mcp_server_final - Security (민감정보 보호 / 안전한 반환)
...
# [mcp_server_final] 설비 Tool 은 DB 우선 + mock fallback 구조라, 데이터 출처 표시용
#   data_source / fallback_used 를 top-level 허용 필드에 추가한다(비민감 요약 필드).
RESULT_FIELD_WHITELIST = {
    "get_equipment_status": (
        "equipment_id", "status_code", "status_label", "updated_at",
    ...
    "get_process_status": (
        "process_name", "line_id", "process_state", "updated_at",
        "data_source", "fallback_used",
    ),
    "get_quality_metrics": (
    ...
    "get_maintenance_history": (
        "equipment_id", "history", "count", "data_source", "fallback_used",
    ),
    "get_equipment_overview": (
        "equipment_id", "equipment_type", "line_id", "data_source", "fallback_used",
    ),
    "search_manual": ("query", "matched_alarm_code", "documents", "retrieval_source", "fallback_used"),
}

# ---------------------------------------------------------------------------
# DB Tool nested 화이트리스트 (list[dict] 응답 내부 key 제한)
# ---------------------------------------------------------------------------
# [용도] DB 조회 결과의 중첩 리스트(recent_events / history) 내부 key 를 제한해,
#        repository 에서 혹시 새어 들어온 민감 컬럼(message/operator_note/lot_id/
#        technician_note 등)이 응답에 남지 않게 2차 방어한다.
#        (1차 방어는 repository 의 컬럼 명시 SELECT 다.)
# [구조] tool_name → {nested 필드명: 허용 내부 key 튜플}
DB_NESTED_WHITELIST = {
    "get_recent_alarm_events": {
        "recent_events": ("alarm_code", "occurred_at", "severity"),
    },
    "get_maintenance_history": {
        "history": ("part_replaced", "action", "performed_at"),
    },
}


def _sanitize_nested_list(items, allowed_keys):
    """list[dict] 의 각 항목에서 허용 key 만 남긴다(마스킹 적용)."""
    ...
    for item in items:
        if not isinstance(item, dict):
            continue
        safe_items.append({k: _mask_value(item[k]) for k in allowed_keys if k in item})
    return safe_items


# ---------------------------------------------------------------------------
# search_manual nested 화이트리스트 (RAG 응답 documents/metadata 내부 key 제한)
# ---------------------------------------------------------------------------
# [용도] search_manual 의 documents[] 와 그 안의 metadata 는 중첩 구조라,
#        top-level 화이트리스트만으로는 내부 key(full_text/source_path 등)를 막지 못한다.
#        그래서 documents 항목과 metadata 에 대해 '내부 key 화이트리스트'를 별도로 둔다.
# [차단] chroma:document(문서 전문), full_text, raw_text, source_path, file_path, path,
#        internal_url, author, owner 등 — 아래 목록에 없으므로 자동 제거된다.
SEARCH_MANUAL_DOCUMENT_FIELDS = (
    "doc_id",
    "title",
    "snippet",
    "score",            # 선택 필드(있을 때만 통과)
    "metadata",
)

SEARCH_MANUAL_METADATA_FIELDS = (
    "doc_name",
    "section_title",
    "alarm_code",
    "equipment_type",
...
def _sanitize_search_manual_metadata(metadata):
    """search_manual document 의 metadata 에서 허용 key 만 남긴다(마스킹 적용)."""
    if not isinstance(metadata, dict):
        return {}
    safe = {}
    for key in SEARCH_MANUAL_METADATA_FIELDS:
        if key in metadata:
            safe[key] = _mask_value(metadata[key])
    return safe


def _sanitize_search_manual_documents(documents):
    """search_manual 의 documents[] 를 항목별 허용 key 로 제한한다(nested whitelist).

    각 document 는 SEARCH_MANUAL_DOCUMENT_FIELDS 만, 그 안의 metadata 는
    SEARCH_MANUAL_METADATA_FIELDS 만 통과시킨다. 나머지(full_text/source_path 등)는 제거.
    """
    ...
    for doc in documents:
        if not isinstance(doc, dict):
            continue
        safe_doc = {}
        for key in SEARCH_MANUAL_DOCUMENT_FIELDS:
            if key not in doc:
                continue
            if key == "metadata":
                safe_doc["metadata"] = _sanitize_search_manual_metadata(doc.get("metadata"))
            else:
                safe_doc[key] = _mask_value(doc[key])
...
def sanitize_result(tool_name, payload):
    ...
    # 3-A) search_manual 의 documents 는 nested whitelist 로 한 번 더 제한한다.
    #      (top-level 마스킹만으로는 documents 내부 full_text/source_path 등을 못 막으므로)
    if tool_name == "search_manual" and "documents" in safe:
        safe["documents"] = _sanitize_search_manual_documents(payload.get("documents"))

    # 3-B) DB Tool 의 중첩 리스트(recent_events / history)도 내부 key whitelist 로 제한한다.
    #      repository 에서 1차로 민감 컬럼을 SELECT 하지 않지만, 여기서 2차 방어를 둔다.
    nested_rules = DB_NESTED_WHITELIST.get(tool_name)
    if nested_rules:
        for field, allowed_keys in nested_rules.items():
            if field in safe:
                safe[field] = _sanitize_nested_list(payload.get(field), allowed_keys)
