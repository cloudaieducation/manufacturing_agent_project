# -*- coding: utf-8 -*-
"""
Day5 mcp_server_final - MCP Server 진입점 (순수 Python 함수형 골격)
...
from day5.mcp_server_final.contracts import ALLOWED_TOOLS, FORBIDDEN_SQL_TOOLS
from day5.mcp_server_final import mcp_schemas
from day5.mcp_server_final import tool_selector
from day5.mcp_server_final import tool_executor
from day5.mcp_server_final import mcp_logging
...
def create_fastmcp_server():
    ...
    """
    try:
        server = FastMCP("mcp_server_final")
...
def warm_up_rag_dependencies():
    ...
    try:
        ...
        from day5.mcp_server_final import manual_search
        manual_search.search_manual("warm up", alarm_code=None)
        print("[mcp_server_final] RAG 의존성 warm-up 완료(메인 스레드 사전 적재)", file=sys.stderr, flush=True)
    except Exception as error:  # noqa: BLE001 - warm-up 실패는 서버 기동을 막지 않는다
        # 사유 원문/스택은 남기지 않고 타입만 짧게(민감정보 방지).
        print(f"[mcp_server_final] RAG 의존성 warm-up 생략({type(error).__name__})", file=sys.stderr, flush=True)
...
def _parse_cli_args(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="mcp_server_final - FastMCP server (stdio 기본, streamable-http 옵션)"
    )
