...
- 이 Streamlit 앱은 기본 실습을 대체하는 도구가 아니라, MCP Tool 호출 흐름을
  화면에서 확인하는 보조 UI입니다. 기준은 CLI 실행 결과와 outputs/day3 산출물입니다.
- 화면에서 "어떤 Tool을, 어떤 입력으로 호출하고, 어떤 응답이 오는지"를 눈으로 확인하고,
  fastmcp(서버 경유)와 fallback(직접 호출) 두 경로의 차이를 비교하는 것이 목적입니다.
- UI 디버깅보다 MCP / Tool 호출 흐름 이해가 우선입니다.
...
def build_tool_input(tool_name: str) -> dict:
    ...
    Tool마다 필요한 입력 키가 다릅니다(= 각 Tool의 입력 스키마/계약).
    그래서 Tool 이름으로 분기해 그 Tool에 맞는 입력 칸만 보여 주고,
    여기서 만든 dict가 곧 call_tool에 넘길 tool_input이 됩니다.
    (기본값 EQP-EV-03/ALM-TEMP-402/LINE-07은 모두 교육용 가상 값입니다.)
    """
...
def main() -> None:
    ...
    # 화면 흐름은 Agent의 Tool 사용 흐름과 같은 순서입니다:
    #   Tool 선택 → 입력값 구성 → 호출 → 결과 확인.
    # 여기서는 사람이 Tool을 직접 고르지만, 4일차에서는 LLM이 description을 보고 고르게 됩니다.
    st.subheader("1. Tool 선택")

    # SUPPORTED_TOOLS(Client 화이트리스트)에 있는 Tool만 고를 수 있습니다.
    tool_name = st.selectbox(
        "호출할 Tool",
        options=SUPPORTED_TOOLS,
    )

    # 선택한 Tool의 설명(TOOL_DESCRIPTIONS)을 함께 보여 줍니다.
    # 이 설명이 곧 "Tool 선택 근거"이며, 설명 품질이 선택 정확도에 영향을 줍니다.
    ...
    if st.button("Tool 호출", type="primary"):
        with st.spinner(f"{tool_name} 호출 중입니다..."):
            # call_tool은 MCP_MODE에 따라 FastMCP 서버 또는 fallback으로 호출을 분배합니다.
            # 화면 코드는 그 경로를 몰라도 되고, Tool 이름과 입력 dict만 넘기면 됩니다.
