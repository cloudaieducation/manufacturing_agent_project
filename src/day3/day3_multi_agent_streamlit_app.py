...
- multi_agent_roles의 Multi-Agent 흐름(Coordinator → DB → RAG → Summary)을
  화면에서 실행하고 결과를 확인하는 보조 UI입니다. 기준은 CLI 실행과 outputs/day3 산출물입니다.
- 화면에서 사용자 질문, 추출된 설비/알람 정보, Agent 실행 단계(trace),
  DB Tool 결과와 RAG Tool 결과(사실 데이터 ↔ 절차 근거를 분리해서 표시), 최종 요약을 확인합니다.
- UI 디버깅보다 Agent 역할 분담과 Tool 호출 흐름 이해가 우선입니다.
...
def show_state_result(state: dict, saved_path: Path | None = None) -> None:
    """
    Multi-Agent 실행 결과(State)를 화면에 표시합니다.

    State의 어느 칸이 화면 어디에 대응하는지 보여 줍니다:
        추출 정보(equipment_id/alarm_code/line_id) → Agent 실행 단계(agent_steps=trace)
        → 최종 요약(final_summary) → Tool 결과(db_results/rag_results).
    특히 DB 결과와 RAG 결과를 분리해 보여 주어, "사실 데이터"와 "절차 근거"를 구분합니다.
    """
...
def main() -> None:
    ...
    # 화면에서 고른 모드를 환경변수(MCP_MODE/MCP_FASTMCP_URL)로 설정합니다.
    # 이 환경변수가 manufacturing_mcp_client.call_tool의 호출 경로(MCP 서버/직접 호출)를 결정하므로,
    # Agent 코드를 고치지 않고 화면 선택만으로 Tool 호출 경로를 바꿀 수 있습니다.
    if mode == "fastmcp":
