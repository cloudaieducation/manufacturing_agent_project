...
[결과를 읽는 법]
- score는 distance를 사람이 읽기 좋게 변환한 참고값일 뿐, 단독 판단 기준이 아닙니다.
...
def distance_to_score(distance: Any) -> float:
    """
    Chroma distance를 수강생이 읽기 좋은 score로 변환합니다.
...
def search_top_k(
    ...
    # Chroma는 여러 질의를 한 번에 처리할 수 있어 결과를 질의별 중첩 리스트로 돌려줍니다.
    # 여기서는 단일 질의만 보냈으므로 첫 번째([0]) 묶음만 꺼내 Top-K 후보로 풀어냅니다.
