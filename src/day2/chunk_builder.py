# -*- coding: utf-8 -*-
"""
2일차 RAG 실습 - Markdown 문서 Chunk 생성기

이 파일은 Day2 RAG 실습에서 Markdown 문서를 검색 가능한 작은 chunk로 나누고,
각 chunk에 metadata를 붙이는 역할만 담당합니다.

실행 흐름:
Markdown 문서 읽기
→ 문단 단위 chunk 생성
→ metadata 생성
→ outputs/day2/chunk_preview.json 저장
→ Mustache 템플릿으로 outputs/day2/chunk_build_result.md 저장

이 파일은 Chroma Vector DB에 저장하지 않습니다.
이 파일은 Ollama embedding을 만들지 않습니다.

Mustache 렌더링에는 pystache가 필요합니다.
...
def get_project_root() -> Path:
    ...
    pathlib.Path는 파일과 폴더 경로를 다루는 Python 도구입니다.
    현재 파일 위치에서 상위 폴더로 올라가며 src 폴더와 docs 또는 outputs 폴더가 있는 위치를 찾습니다.
    """
...
def build_chunk_records(docs_dir: Path) -> List[Dict[str, str]]:
    """
    docs 폴더의 Markdown 문서를 읽고 chunk record 목록을 생성합니다.

    초보자용 예제이므로 yield나 복잡한 내부 처리 대신 단순한 for 반복문으로 작성합니다.
    문단은 빈 줄 기준으로 나눕니다.
    """
