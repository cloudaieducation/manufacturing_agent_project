"""
Day4 LLM Tool Selector - real LLM 전용 버전
...
def build_tool_arguments(tool_name, equipment_id, alarm_code, user_query):
    """
    선택된 Tool에 Tool Contract(인자 계약)에 맞는 arguments를 채워 줍니다.

    Tool Contract 강화 포인트:
    - get_equipment_status / get_process_status / get_quality_metrics /
      get_maintenance_history : equipment_id(가능 시) + limit
    - get_recent_alarm_events : equipment_id + alarm_code(가능 시) + limit
    - search_manual : 검증기 계약상 "alarm_code 또는 query"가 반드시 있어야 하므로,
      alarm_code가 있으면 함께 넣고 query(원본 질문)는 항상 채워 계약을 충족시킨다.
      (기존 코드는 query를 한 번도 채우지 않아 alarm_code가 없는 질문에서 인자 누락이 발생했음)
    """
    arguments = {}
    ...
    if tool_name == "search_manual":
        if alarm_code:
            arguments["alarm_code"] = alarm_code
        # query는 항상 채워 "alarm_code 또는 query" 계약을 보장한다.
        if user_query:
    ...
    for case in cases:
        ...
        if guardrail_result is not None:
            ...
            for tool_name in selected_tools:
                arguments = build_tool_arguments(tool_name, equipment_id, alarm_code, user_query)
                tool_plan.append({
                    "tool_name": tool_name,
