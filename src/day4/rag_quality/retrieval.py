# -*- coding: utf-8 -*-
"""
Day4 RAG 검색 품질 평가기 - 검색 계층 모듈 (retrieval)

이 파일은 "질문을 받아 관련 문서 chunk를 찾아오는" 검색 단계만 모아 둡니다.
- Ollama 임베딩 호출(_embed_query_with_ollama)
- 실제 Chroma Vector DB 검색(chroma_retrieve_chunks)
- 교육용 가짜 검색(mock_retrieve_chunks)
- Chroma 우선, 실패 시 mock으로 자동 전환하는 통합 검색(retrieve_chunks)

교육용 리팩토링 3단계에서 runner.py로부터 분리했습니다.
- 함수 내부 로직은 분리 전과 한 글자도 다르지 않습니다(안전한 위치 이동).
- rerank / scoring / report / 판정(PASS/WARN/FAIL) 로직은 이 파일에 들어오지 않습니다.

import 계층 규칙(중요):
- retrieval.py는 config.py만 import 합니다(runner.py를 import 하지 않습니다).
